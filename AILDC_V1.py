# -*- coding: utf-8 -*-
"""
Created on Wed Dec 11 13:22:40 2024
    ...
    if where == gp.GRB.Callback.MIPSOL:                     # when find an integer solution
        ...
        # Disjuntive cut procedure and calculate Qip -------------------------------------------------------------------------------------------------
            ...
            if (thetaval - Qip) / max(Qip, 1e-5) > 1e-5 :
                model._cut_coef_list.append((final_beta, final_alpha))      # store final cut coeff. to list
...
#%% Results
print('Opt+Setup Time AILDC_V1', timeAILDC)
print('Optimization Time AILDC_V1', timeOAILDC)    
...
with open("gurobi_statistics_AILDCV1.txt", "w") as file:
    file.write("Scenarios, Method, Nodes Explored, Opt Time, Opt+Setup Time, Optimality Gap, Master Obj, Fixed Costs, 2-Stage OF, r\n")
    file.write(f"{N}, AILDCV1, {nodes_explored}, {timeOAILDC}, {timeAILDC}, {gap}, {OF}, {Fixed_costs}, {Second_Stage_OF}, {r}\n")
