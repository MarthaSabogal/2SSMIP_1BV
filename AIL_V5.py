# -*- coding: utf-8 -*-
"""
Created on Sun Feb 16 15:19:57 2025
...
Jhat1, Jhat0 = {}, {}       # to store j where y_j=1 and y_j=0
alpha, beta = {}, {}        # to store cut coeff.
subp_obj = {}               # to store subp obj for scenario

# Algorithm with callback and lazy constraints
def callback(model, where):
    if where == gp.GRB.Callback.MIPSOL:                     # when find an integer solution
        ...
        for w in range(N):
            ...
            if (z.x >= 1 - 1e-5) or (z.x <= 0 + 1e-5):            # Identify the scenarios where z was integer
                QlpInteger += (1/N)*m_subp.objVal
                ListQlpInteger.append(w)
            
        if (thetaval - Qlp)/max(Qlp, 1e-5) > 1e-5:              # Benders cut violation
            model.cbLazy(model._theta <= (1/N)*sum(beta[w]+sum(sum(alpha[w][j,l]*model._y[j,l] for j in J) for l in L) for w in range(N)))     # Benders cut
            model.update()
            
            if len(ListQlpInteger) == N:
                model.cbLazy(model._theta <= Qlp + (U - Qlp)*(sum(model._y[j,l] for (j,l) in Jhat0)+sum((1-model._y[j,l]) for (j,l) in Jhat1)))  # Integer L-shaped cut      
                model.update()
            
        else:
            AuxSet = [aux for aux in range(N) if aux not in ListQlpInteger]            # Scenarios where z was not integer
            
            # Solve Subproblem as 2LPs ----------------------------------------------------------------------------------------------------------
            Qip = QlpInteger
            for w in AuxSet:
                update_rhs(d, q_s, q_p, gamma_s, gamma_p, eta, xi_nd, xi_qs, xi_qp, M3, yval, w)
            ...
            if (thetaval - Qip) / max(Qip, 1e-5) > 1e-5:
                model.cbLazy(model._theta <= Qip + (U - Qip)*(sum(model._y[j,l] for (j,l) in Jhat0)+sum((1-model._y[j,l]) for (j,l) in Jhat1)))  # Integer L-shaped cut        
                model.update()
...
#%% Results
print('Opt+Setup Time AIL_V5', timeAIL)
print('Optimization Time AIL_V5', timeOAIL)    
...
with open("gurobi_statistics_AILV5.txt", "w") as file:
    file.write("Scenarios,Method,Nodes Explored,Opt Time,Opt+Setup Time,Optimality Gap,Master Obj,Fixed Costs,2-Stage OF,r\n")
    file.write(f"{N},AILV5,{nodes_explored},{timeOAIL},{timeAIL},{gap},{OF},{Fixed_costs},{Second_Stage_OF},{r}\n")
