# -*- coding: utf-8 -*-
"""
Created on Sun Feb 16 14:32:56 2025
...
def callback(model, where):
    if where == gp.GRB.Callback.MIPSOL:                     # when find an integer solution
        ...
        if (thetaval - Qlp)/max(Qlp, 1e-5) > 1e-5:
            ...
            if len(ListQlpInteger0)+len(ListQlpInteger1) == N:
        ...
        # Disjuntive cut procedure and calculate Qip -------------------------------------------------------------------------------------------------
            ...
            if (thetaval - Qip) / max(Qip, 1e-5) > 1e-5:
                ...
                model.cbLazy(model._theta <= (1/N)*sum(final_beta[w]+sum(sum(final_alpha[j,l,w]*model._y[j,l] for j in J) for l in L)   # Disjunctive cut
                                                           for w in range(N)))
                model.update()
...
ini_time_OAILDC = time.time()
m_mp.optimize(callback)
timeOAILDC = time.time()-ini_time_OAILDC        # Optimization time
timeAILDC = time.time()-ini_time_AILDC          # Optimization + setup of models

#%% Results
print('Opt+Setup Time AILDC_V2', timeAILDC)
print('Optimization Time AILDC_V2', timeOAILDC)    
...
with open("gurobi_statistics_AILDCV2.txt", "w") as file:
    file.write("Scenarios, Method, Nodes Explored, Opt Time, Opt+Setup Time, Optimality Gap, Master Obj, Fixed Costs, 2-Stage OF, r\n")
    file.write(f"{N}, AILDCV2, {nodes_explored}, {timeOAILDC}, {timeAILDC}, {gap}, {OF}, {Fixed_costs}, {Second_Stage_OF}, {r}\n")
