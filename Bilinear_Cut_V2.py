# -*- coding: utf-8 -*-
"""
Created on Tue Mar 18 14:54:30 2025
...
filename = f"gurobi_stats_ABCV2_M_C_II_{se}_{int(r*100)}.txt"
print(filename)

#%% Subproblem LP
ini_time_ABC = time.time()
...
def callback(model, where):    
    if where == gp.GRB.Callback.MIPSOL:                                             # when find an integer solution
        ...
        yval = {}
        alpha_Qlp, beta_Qlp = {}, {}                                                # to store benders cut coeff.
        subp_obj = {}
        Zcoe, Ycoe, Wcoe, B_0 = {}, {}, {}, {}                                      # to store bilinear cut coeff.
            ...
            if (thetaval[w] - Qlp)/max(Qlp, 1e-5) > 1e-5:
                ...
                if z.x <= 1e-5:                      # z lp was 0                
                    ...
                    if m_subp.status == GRB.OPTIMAL:      
                        ...
                        Zcoe[w] = 0 if abs(t_beta1 - t_beta0) < 1e-6 else (t_beta1 - t_beta0)
                        for j in J:
                            for l in L:
                                Ycoe[j,l,w] = t_alpha0[j,l]
                                Wcoe[j,l,w] = 0 if abs(t_alpha1[j,l] - t_alpha0[j,l]) < 1e-6 else (t_alpha1[j,l] - t_alpha0[j,l])
                        ...
                        if m_subp.status == GRB.INFEASIBLE:
                            model.cbLazy(model._Z[w] <= len(Jhat1) - sum(model._y[j,l] for (j,l) in Jhat1) + sum(model._y[j,l] for (j,l) in Jhat0))  # feasibility cut                
                    ...
                    B_0[w] = 0 if abs(t_beta0) < 1e-6 else t_beta0
                    Zcoe[w] =  0 if abs(t_beta1 - t_beta0) < 1e-6 else (t_beta1 - t_beta0)
                    for j in J:
                        for l in L:
                            Ycoe[j,l,w] = 0 if abs(t_alpha0[j,l]) < 1e-6 else t_alpha0[j,l]
                            Wcoe[j,l,w] = 0 if abs(t_alpha1[j,l] - t_alpha0[j,l]) < 1e-6 else (t_alpha1[j,l] - t_alpha0[j,l])
                    ...
                    if m_subp.status == GRB.OPTIMAL:      
                        ...
                        subp_obj[w] = obj_spz0                        
                        Zcoe[w] = 0 if abs(-t_beta0) < 1e-6 else -t_beta0
                        for j in J:
                            for l in L:
                                Ycoe[j,l,w] = 0 if abs(t_alpha0[j,l]) < 1e-6 else t_alpha0[j,l]
                                Wcoe[j,l,w] = 0 if abs(-t_alpha0[j,l]) < 1e-6 else -t_alpha0[j,l]
                ...
                if (thetaval[w] - subp_obj[w])/max(subp_obj[w], 1e-5) > 1e-5:
                    model.cbLazy(model._theta[w] <= B_0[w] + Zcoe[w]*model._Z[w] + sum(sum(Ycoe[j,l,w]*model._y[j,l] + Wcoe[j,l,w]*model._W[j,l,w] for j in J) for l in L))   # Bilinear cut
...
ini_time_OABC = time.time()
m_mp.optimize(callback)
timeOABC = time.time()-ini_time_OABC        # Optimization time
timeABC = time.time()-ini_time_ABC          # Optimization + setup of models

#%% Results
print('Opt+Setup Time ABC_V2_M', timeABC)
print('Optimization Time ABC_V2_M', timeOABC)
...
with open(filename, "w") as file:
    ...
    file.write(f"{N},ABCV2_M_C_II,{nodes_explored},{timeOABC},{timeABC},{gap},{OF},{Fixed_costs},{Second_Stage_OF},{r},{se},{m_mp.status}\n")
