# -*- coding: utf-8 -*-
"""
Created on Tue Feb 11 18:27:57 2025
...
def callback(model, where):
    if where == gp.GRB.Callback.MIPSOL:                     # when find an integer solution
        ...
        if (thetaval - Qlp)/max(Qlp, 1e-5) > 1e-5:
            ...
            AuxSet = [aux for aux in range(N) if aux not in ListQlpInteger]           # Scenarios that z was not integer
            ...
            if (thetaval - Qip) / max(Qip, 1e-5) > 1e-5:
                model.cbLazy(model._theta <= Qip + (U - Qip)*(sum(model._y[j,l] for (j,l) in Jhat0)+sum((1-model._y[j,l]) for (j,l) in Jhat1)))            
                model.update()
...
#%% Results
print('Opt+Setup Time AIL_V4', timeAIL)
print('Optimization Time AIL_V4', timeOAIL)    
...
with open("gurobi_statistics_AILV4.txt", "w") as file:
    file.write("Scenarios,Method,Nodes Explored,Opt Time,Opt+Setup Time,Optimality Gap,Master Obj,Fixed Costs,2-Stage OF,r\n")
    file.write(f"{N},AILV4,{nodes_explored},{timeOAIL},{timeAIL},{gap},{OF},{Fixed_costs},{Second_Stage_OF},{r}\n")
