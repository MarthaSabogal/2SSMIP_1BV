# -*- coding: utf-8 -*-
"""
Created on Tue Feb 11 14:27:33 2025
...
def callback(model, where):
    if where == gp.GRB.Callback.MIPSOL:                     # when find an integer solution
        ...
        if (thetaval - Qlp)/max(Qlp, 1e-5) > 1e-5:      # Benders cut violation
            model.cbLazy(model._theta <= (1/N)*sum(beta[w]+sum(sum(alpha[w][j,l]*model._y[j,l] for j in J) for l in L) for w in range(N)))
            model.update()
...
#%% Results
print('Opt+Setup Time AIL_V2', timeAIL)
print('Optimization Time AIL_V2', timeOAIL)    
...
with open("gurobi_statistics_AILV2.txt", "w") as file:
    file.write("Scenarios,Method,Nodes Explored,Opt Time,Opt+Setup Time,Optimality Gap,Master Obj,Fixed Costs,2-Stage OF,r\n")
    file.write(f"{N},AILV2,{nodes_explored},{timeOAIL},{timeAIL},{gap},{OF},{Fixed_costs},{Second_Stage_OF},{r}\n")
