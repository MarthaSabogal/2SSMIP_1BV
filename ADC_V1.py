# -*- coding: utf-8 -*-
"""
Created on Tue Jan  7 19:58:52 2025
    ...
    if where == gp.GRB.Callback.MIPSOL:                             # when find an integer solution
        ...
        # Disjuntive cut procedure and calculate Qip -------------------------------------------------------------------------------------------------
            ...
            if (thetaval - Qip) / max(Qip, 1e-5) > 1e-5:
                model._cut_coef_list.append((final_beta, final_alpha))                         # store final cut coeffs. to list
...
#%% Results
print('Opt+Setup Time ADC_V1', timeADC)
print('Optimization Time ADC_V1', timeOADC)    
...
with open("gurobi_statistics_ADCV1.txt", "w") as file:
    file.write("Scenarios,Method,Nodes Explored,Opt Time,Opt+Setup Time,Optimality Gap,Master Obj,Fixed Costs,2-Stage OF,r\n")
    file.write(f"{N},ADCV1,{nodes_explored},{timeOADC},{timeADC},{gap},{OF},{Fixed_costs},{Second_Stage_OF},{r}\n")
